...
static void main_loop(AppState *app) {
    ...
    while (g_running) {
         ...
         */
        {
             ...
             */
            int srt_max_buf = (OUT_SAMPLE_RATE * 150) / 1000; /* ~6615 samples @ 44.1kHz */

            /* Trim SRT FIFO once per tick (not per audio frame) */
            if (audio_mode == AUDIO_SRT) {
                pthread_mutex_lock(&sh->lock);
                int excess = av_audio_fifo_size(sh->audio_fifo) - srt_max_buf;
                if (excess > 0) {
                    uint8_t *junk[8] = {0};
                    av_samples_alloc(junk, NULL, OUT_CHANNELS, excess, AV_SAMPLE_FMT_FLTP, 0);
                    av_audio_fifo_read(sh->audio_fifo, (void **)junk, excess);
                    av_freep(&junk[0]);
                }
                pthread_mutex_unlock(&sh->lock);
            }

            /* Bulk-copy SRT audio to local scratch once per tick */
            AVAudioFifo *srt_local = NULL;
            if (audio_mode == AUDIO_SRT) {
                srt_local = app->bg_audio_fifo;  /* reuse as scratch */
                av_audio_fifo_reset(srt_local);
                pthread_mutex_lock(&sh->lock);
                ...
                if (avail > 0) {
                    ...
                    av_audio_fifo_write(srt_local, (void **)tbuf, avail);
                    av_freep(&tbuf[0]);
                }
                pthread_mutex_unlock(&sh->lock);
            }
            ...
            while (app->out.audio_pts < target_audio) {
                switch (audio_mode) {
                case AUDIO_SRT:
                    /* encode_one_audio_frame pads with silence if not enough */
                    encode_one_audio_frame(app, srt_local, aframe_sz);
                    break;
                case AUDIO_GRACE: {
                    AVAudioFifo *scratch = app->bg_audio_fifo;
                    av_audio_fifo_reset(scratch);
                    encode_one_audio_frame(app, scratch, aframe_sz);
                    pthread_mutex_lock(&sh->lock);
                    av_audio_fifo_reset(sh->audio_fifo);
                    pthread_mutex_unlock(&sh->lock);
                    break;
